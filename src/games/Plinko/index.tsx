...
import logoImg from './logo.png' // Adjust the path to your logo image
...
const BET = [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 3, 3, 3, 3, 3, 3, 3, 6]
...
export default function Plinko() {
  ...
  // Ref to hold the loaded logo image
  const logo = React.useRef<HTMLImageElement | null>(null)

  // Load the logo image when the component mounts
  React.useEffect(() => {
    const img = new Image()
    img.src = logoImg
    logo.current = img
            ...
            if (debug) {
              ...
              bodies.forEach((body, i) => {
                ...
                if (label === 'Plinko') {
                  ctx.save()
                  ctx.translate(position.x, position.y)
                  ctx.rotate(angle) // Rotate the context by the body's angle

                  const radius = PLINKO_RAIUS
                  const diameter = radius * 2

                  if (logo.current && logo.current.complete) {
                    ctx.drawImage(logo.current, -radius, -radius, diameter, diameter)
                  } else {
